...
class plcr(base_estimation):
    ...
    def calibration(self, glints,points,center):
        pass
    ...
    def get_plane(self):
        ...
        def get_func(x):
            ...
            return [(((self._glints[:, 0].reshape((3, 1)) + b1 * I[0]) - (
                        self._glints[:, 1].reshape((3, 1)) + b2 * I[1])) ** 2).sum() - self._w ** 2,
                    (((self._glints[:, 2].reshape((3, 1)) + b3 * I[2]) - (
                                self._glints[:, 3].reshape((3, 1)) + b4 * I[3])) ** 2).sum() - self._w ** 2,
                    (((self._glints[:, 0].reshape((3, 1)) + b1 * I[0]) - (
                                self._glints[:, 2].reshape((3, 1)) + b3 * I[2])) ** 2).sum() - self._d ** 2,
                    (((self._glints[:, 1].reshape((3, 1)) + b2 * I[1]) - (
                                self._glints[:, 3].reshape((3, 1)) + b4 * I[3])) ** 2).sum() - self._d ** 2]
        # def get_func2(x):
        #     b1, b2, b3, b4 = x[0], x[1], x[2], x[3]
        #     return [(((self._glints[:, 0].reshape((3, 1)) + b1 * I[0]) - (
        #                 self._glints[:, 1].reshape((3, 1)) + b2 * I[1])) ** 2).sum() - self._w ** 2,
        ...
        #                         self._glints[:, 3].reshape((3, 1)) + b4 * I[3])) ** 2).sum() - self._w ** 2,
        #             (((self._glints[:, 0].reshape((3, 1)) + b1 * I[0]) - (
        #                         self._glints[:, 3].reshape((3, 1)) + b4 * I[3])) ** 2).sum() - self._h ** 2,
        #             (((self._glints[:, 1].reshape((3, 1)) + b2 * I[1]) - (
        #                         self._glints[:, 2].reshape((3, 1)) + b3 * I[2])) ** 2).sum() - self._h ** 2]
        # def get_func3(x):
        #     b1, b2, b3, b4 = x[0], x[1], x[2], x[3]
        #     return [(((self._glints[:, 0].reshape((3, 1)) + b1 * I[0]) - (
        #                 self._glints[:, 3].reshape((3, 1)) + b4 * I[3])) ** 2).sum() - self._h ** 2,
        #             (((self._glints[:, 2].reshape((3, 1)) + b3 * I[2]) - (
        #                         self._glints[:, 1].reshape((3, 1)) + b2 * I[1])) ** 2).sum() - self._h ** 2,
        #             (((self._glints[:, 0].reshape((3, 1)) + b1 * I[0]) - (
